package com.zhenglee.framework.network.request;

import android.support.annotation.NonNull;

import com.zhenglee.framework.network.OkRequest;
import com.zhenglee.framework.network.cache.CacheEntity;
import com.zhenglee.framework.network.cache.CacheManager;
import com.zhenglee.framework.network.cache.CacheMode;
import com.zhenglee.framework.network.callback.AbsCallback;
...
import com.zhenglee.framework.network.utils.HeaderParser;
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    public BaseRequest(String url) {
        ...
        //添加公共请求参数
        if (okRequest.getCommonParams() != null) {
            params.put(okRequest.getCommonParams());
        }
        if (okRequest.getCommonHeaders() != null) {
            headers.put(okRequest.getCommonHeaders());
        }
        //添加缓存模式
        if (okRequest.getCacheMode() != null) {
            cacheMode = okRequest.getCacheMode();
        }
    }
    ...
    protected Call generateCall(Request request) {
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0 && certificates == null) {
            return OkRequest.getInstance().getOkHttpClient().newCall(request);
        } else {
            OkHttpClient.Builder newClientBuilder = OkRequest.getInstance().getOkHttpClient().newBuilder();
