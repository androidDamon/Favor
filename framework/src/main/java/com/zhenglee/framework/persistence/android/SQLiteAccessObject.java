...
public abstract class SQLiteAccessObject<T extends PersistentObject> extends ContentProvider implements DataAccessObject<T> {
    ...
    public long count(String sql, String... args) throws PersistenceException {
        ...
        try {
            c = this.rawQuery("SELECT * FROM " + getTableName() + " WHERE " + sql, args);
            if (null != c && c.moveToNext()) {
                return c.getLong(0);
            }
    ...
    public boolean exists(String id) throws PersistenceException {
        final String sql = "SELECT COUNT(*) FROM " + getTableName() + " WHERE " + BaseColumns._ID + "=?";
    ...
    protected long insertOrThrow(String table, String nullColumnHack, ContentValues values) throws SQLException {
        final SQLitePersistenceManager pm = getPersistenceManager();
        final SQLiteDatabase db = pm.getWritableDatabase();
        return db.insertOrThrow(table, nullColumnHack, values);
    }

    protected long insertOrThrow(String table, String nullColumnHack, ContentValues values, int conflictAlgorithm) {
        final SQLitePersistenceManager pm = getPersistenceManager();
        final SQLiteDatabase db = pm.getWritableDatabase();
    ...
    protected int updateWithOnConflict(String table, ContentValues values, String whereClause, String[] whereArgs, int conflictAlgorithm) {
        final SQLitePersistenceManager pm = getPersistenceManager();
        final SQLiteDatabase db = pm.getWritableDatabase();
