...
import com.zhenglee.framework.persistence.PersistenceException;
...
public class ORMLiteAccessObject<T extends PersistentObject> implements DataAccessObject<T>, RawRowMapper<T> {
    ...
    public ORMLiteAccessObject(ORMLitePersistenceSupport support, Class<T> clazz) {
        ...
        try {
            ...
            this.tableInfo = new TableInfo<T, String>(support.getConnectionSource(), this.dao, clazz);
    ...
    @Override
    public long count(String clauses, String... args) throws PersistenceException {
        final String sql = new StringBuilder("SELECT COUNT(*) FROM ").append(this.tableInfo.getTableName()).append(" WHERE ").append(clauses).toString();
        try {
            final GenericRawResults<String[]> results = this.dao.queryRaw(sql, args);
    ...
    public void create(final T... pos) throws PersistenceException {
        if (null == pos || pos.length < 0) return;
    ...
    @Override
    public <V> V execute(final Callable<V> transaction) throws PersistenceException {
        try {
            return TransactionManager.callInTransaction(this.support.getConnectionSource(), transaction);
        } catch (SQLException e) {
            throw new PersistenceException(e);
        }
    }

    public List<T> query(String clauses, String... args) throws PersistenceException {
        ...
        try {
            final GenericRawResults<T> results = this.dao.queryRaw(sql.toString(), this, args);
